...
def dom_wdeg(assignment, csp):
    a = 1000000
    sum = dict()   #edw apothikeuontai oi metavlhtes ws keys kai me values ta weights toys 
    result=None   
    ...
    for var in csp.variables:
        if var not in assignment:
            sum[var]=0   #arxikopoiountai me 1 ola ta sums
            for neighbor in csp.neighbors[var]:
                if neighbor not in assignment:
                    sum[var]+=csp.weight[(var, neighbor)]     #kanw add sto sum to weight tou var pou exei me kathe neighbor
            #sto domain_size kratame ton arithmo twn domains ths var
            domain_size = len(csp.curr_domains[var])   
            #ean to phliko domain_size/sum einai mikrotero apo to a apothikeuetai kai epistrefetai
            if sum[var]==0:
                continue
